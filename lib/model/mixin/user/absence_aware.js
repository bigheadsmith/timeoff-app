...
module.exports = function(sequelize) {
            ...
            [Op.or]: {
              date_start: {
                [Op.between]: [
                  ...
                  moment.utc(year.clone().add(is_multi_year ? 1 : 0, 'years')).endOf('year').format('YYYY-MM-DD 23:59:59')
                ]
              },
              date_end: {
                [Op.between]: [
  ...
  this.promise_my_leaves = function(args) {
        ...
        [Op.or]: {
          date_start: {
            [Op.between]: [
              ...
              moment.utc().endOf('year').format('YYYY-MM-DD 23:59:59')
            ]
          },
          date_end: {
            [Op.between]: [
        ...
        [Op.or]: [
          {
            date_start: {
              [Op.between]: [
                ...
                moment.utc(dateEnd).endOf('day').format('YYYY-MM-DD 23:59:59')
              ]
            }
          },
          {
          ...
          },
          {
            [Op.and]: [
              {
                date_start: {
                  [Op.lte]: moment.utc(dateStart).startOf('day').format('YYYY-MM-DD')
                }
              },
              {
                date_end: {
                  [Op.gte]: moment.utc(dateEnd).endOf('day').format('YYYY-MM-DD 23:59:59')
                }
              }
            ]
          }
    ...
    console.log('promise_my_leaves where_clause:', where_clause);
  ...
  this.promise_my_active_leaves = function(args) {
    try {
      ...
      return this.promise_my_leaves({
        ...
        console.log(`Successfully retrieved and sorted leaves for year: ${year}`);
  ...
  this.calculate_number_of_days_taken_from_allowance = function(args) {
    let leave_type = args ? args.leave_type : null;
    let leaves_to_traverse = this.my_leaves || [];
    let count_personal_only = args ? args.count_personal : false;
    ...
    if (count_personal_only) {
      leaves_to_traverse = leaves_to_traverse.filter(leave => leave.leave_type && leave.leave_type.use_personal);
    } else if (leave_type) {
      leaves_to_traverse = leaves_to_traverse.filter(leave =>
        leave_type.use_personal ? leave.leave_type && leave.leave_type.use_personal : leave.leaveTypeId === leave_type.id
      );
    }

    // Calculate the sum of deducted days for approved leaves
    let result = leaves_to_traverse.reduce((memo, leave) => {
  ...
  this.get_leave_statistics_by_types = function(args) {
    ...
    let statistics = {}
    let limit_by_top = args.limit_by_top || false;
    ...
    _.filter(
      ...
      .forEach(
        function(leave) {
          ...
          let stat_obj = statistics[leave.leave_type.id];
