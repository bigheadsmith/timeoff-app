...
module.exports = function(sequelize) {
  ...
  this.calculate_number_of_days_taken_from_allowance = function(args) {
    ...
    let count_personal_only = args ? args.count_personal : null;
    ...
    if (count_personal_only) {
      ...
      // or if leave_type uses personal, count all leaves that use personal
      if (leave_type) {
        if (leave_type.use_personal) {  // can't read internals if no leave_type provided
          leaves_to_traverse = _.filter(
            leaves_to_traverse,
            leave => leave.leave_type.use_personal
          );
