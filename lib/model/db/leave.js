...
module.exports = function(sequelize, DataTypes) {
  ...
  Leave.create_new_leave = function(args) {
    ...
    return Promise.try(() => employee.validate_overlapping(valide_attributes))
      ...
      .then(main_supervisor => {
        const start_date = moment.utc(valide_attributes.from_date).format('YYYY-MM-DD')
        const end_date = moment.utc(valide_attributes.to_date).format('YYYY-MM-DD')
  ...
  Leave.prototype.get_days = function() {
    ...
    const start_date = moment.utc(self.date_start);
    const end_date = moment.utc(self.date_end);
    let days = [start_date.format('YYYY-MM-DD')];

    console.log("db.leave start_date", start_date.format('YYYY-MM-DD'));

    if (!start_date.isSame(end_date, 'day')) {
      const days_in_between = end_date.diff(start_date, 'days') - 1;

      for (let i = 1; i <= days_in_between; i++) {
        days.push(moment.utc(start_date).add(i, 'days'));

      }
      days.push(end_date.format('YYYY-MM-DD'));
    }
    ...
    days = _.map(
      ...
      day =>
        new LeaveDay({
          ...
          sequelize,
          date: moment.utc(day).format('YYYY-MM-DD'),
          day_part: moment.utc(day).isSame(start_date, 'day')
            ? self.day_part_start
            : moment.utc(day).isSame(end_date, 'day')
              ? self.day_part_end
