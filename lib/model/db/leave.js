...
module.exports = function(sequelize, DataTypes) {
  ...
  Leave.prototype.get_days = function() {
    ...
    if (start_date !== end_date) { // Use the formatted strings for comparison
      const days_in_between = moment.utc(end_date).diff(moment.utc(start_date), 'days') - 1;

      for (let i = 1; i <= days_in_between; i++) {
        days.push(moment.utc(start_date).add(i, 'days').format('YYYY-MM-DD'));
      }
