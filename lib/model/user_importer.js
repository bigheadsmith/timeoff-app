...
async function add_users_in_bulk({ bulk_header, bulk_data, to_company_id }) {
  const company = await Models.Company.scope('with_simple_departments').findOne({ where: { id: to_company_id } });
  ...
  const dep_name_to_id = _.fromPairs(company.departments.map(dep => [dep.name, dep.id]));
  const with_invalid_departments = bulk_data.filter(vector => !dep_name_to_id[vector[department_vector_index]]);
  if (with_invalid_departments.length > 0) {
    const unknown_departments = with_invalid_departments.map(vector => `"${vector[department_vector_index]}"`).join(', ');
    throw new Error(`Unknown departments: ${unknown_departments}`);
  }

  bulk_data.forEach(vector => {
    vector[department_vector_index] = dep_name_to_id[vector[department_vector_index]];
  });
