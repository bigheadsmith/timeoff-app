...
class UserAllowance {
  ...
  get total_personal_taken() {
    const self = this;
    let personal_taken = self._user.calculate_number_of_days_taken_from_allowance({
      year: self._now.year(),
      count_personal: true,
    })
  ...
  get number_of_regular_allowance() {
    ...
    let regular_allowance = self.total_number_of_days_in_allowance -
      self.nominal_personal
  ...
  get employement_range_adjustment() {
    ...
    const start_date = moment.utc(self.user.start_date).year() === now.year()
      ? moment.utc(self.user.start_date)
      : now.clone().startOf('year');

    // Determine end_date based on the user's end date and current year
    const end_date = self.user.end_date && moment.utc(self.user.end_date).year() <= now.year()
    ...
    const proratedAllowance = Math.round((self.nominal_allowance * daysDifference) / 365);
