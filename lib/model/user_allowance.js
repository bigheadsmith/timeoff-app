...
class UserAllowance {
  constructor(args) {
    ...
    self._nominal_personal = args.nominal_pesonal
    self._now = args.now
  }
  ...
  get total_personal_available() {
    ...
    return (self._nominal_personal - personal_taken)
  };
  ...
  get employement_range_adjustment() {
    ...
    const start_date =
      moment.utc(self.user.start_date).year() === now.year()
        ? moment.utc(self.user.start_date).format('YYYY-MM-DD') // adding
        : now.clone().startOf('year')
    const end_date =
      self.user.end_date && moment.utc(self.user.end_date).year() <= now.year()
        ? moment.utc(self.user.end_date).format('YYYY-MM-DD')   // adding
