  ...
  req.user
    ...
    .then(() =>
      UserImporter.add_user({
        ...
        work_hours: new_user_attributes.work_hours,
        break_times: new_user_attributes.break_times,
        employee_num: new_user_attributes.employee_num
      })
    )
...
function promise_user_list_data_for_rendering(args) {
  ...
  const usersInfoForRendering = users_info.map(ui => ({
    ...
    user_work_hours: ui.user_row.work_hours,
    user_break_times: ui.user_row.break_times,
    user_employee_num: ui.user_row.employee_num
  }))
...
function get_and_validate_user_parameters(args) {
  ...
  const work_hours = params.work_hours && params.work_hours.split('\n').map(line => line.trim()).join('\n');
  const break_times = params.break_times && validator.trim(params.break_times)
  const employee_num = params.employee_num && validator.trim(params.employee_num)
