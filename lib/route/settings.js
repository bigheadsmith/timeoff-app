...
router.post('/carryOverUnusedAllowance/', (req, res) => {
  req.user
    ...
    .catch(error => {
      ...
      console.log(
        `[${logMarker}] An error occurred while trying to carry over unused allowance by user ${req.user.id
...
function get_and_validate_leave_type(args) {
  ...
  if (!validator.isHexadecimal(color.replace('#', ''))) {
    throw_user_error(
      'New color for ' + item_name + ' should be a valid hexadecimal color value'
    )
  }

  if (typeof limit !== 'number' && (!limit || !validator.isNumeric(limit))) {
    throw_user_error(
      'New limit for ' + item_name + ' should be a valid number'
    )
  } else if (limit < 0) {
    throw_user_error(
...
router.post('/company/delete/', (req, res) => {
  ...
  req.user
    ...
    .catch(error => {
      console.log(
        `Failed to remove ${company.id} by user ${req.user.id
