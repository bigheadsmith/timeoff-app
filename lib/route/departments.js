...
function get_and_validate_department(args) {
  ...
  const allowance = getParam('allowance') ? validator.trim(getParam('allowance')) : '';
  const manager_id = getParam('manager_id') ? validator.trim(getParam('manager_id')) : '';

  const include_public_holidays = validator.toBoolean(getParam('include_public_holidays'));
  console.log('include_public_holidays:', include_public_holidays)
  const is_accrued_allowance = getParam('is_accrued_allowance') ? validator.toBoolean(getParam('is_accrued_allowance')) : false;

  // Validate provided parameters
  //
  // New allowance should be from range of (0;50]
  if (!validator.isFloat(allowance)) {
    session.flash_error('New allowance for ' + department_name + ' should be numeric');
  } else {
