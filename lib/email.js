...
console.log('SMTP configuration:', config.get('smtp'))
...
function _promise_to_unfold_context(context) {
  // Ensure that context.user is defined and has the method reload_with_session_details
  if (
    context.user &&
    typeof context.user.reload_with_session_details === 'function'
  ) {
    return context.user
      .reload_with_session_details()
      .then(updatedUserData => {
        // Assuming reload_with_session_details updates the user's data and returns it
        if (updatedUserData) {
          context.user = { ...context.user, ...updatedUserData }
        }
        return context // Return the updated context
      })
      .catch(error => {
        console.error('Error unfolding context: ', error)
        return context // Return the original context in case of error
      })
  } else {
    console.log(
      'User object is undefined or reload_with_session_details is not a function'
    )
    return bluebird.resolve(context) // Return the original context if user or method is not defined
  }
}
...
Email.prototype.promise_rendered_email_template = function(args) {
  ...
  console.log('Rendering email template:', filename)
  const { leave, user } = args.context
  ...
  // First, render the template to extract the subject
  return handlebars
    .render(__dirname + '/../views/email/' + filename + '.hbs', context)
    .then(text => {
      const subject_and_body = text.split(/\r?\n=====\r?\n/)
      const subject = subject_and_body[0]
      const body = subject_and_body[1]

      // Continue with unfolding context and other processing
      return _promise_to_unfold_context(context).then(unfoldedContext => {
        // Use the unfolded context for further processing or additional rendering
        // But since we already have the subject and body, we might not need to render again
        return handlebars
          .render(__dirname + '/../views/email/wrapper.hbs', {
            subject: subject,
            body: body,
            context: unfoldedContext
          })
          .then(final_email => {
            return {
              subject: subject,
...
Email.prototype.promise_add_new_user_email = function(args) {
  ...
  // console.log('Context add_new_user: ', context)
  ...
  return self
    ...
    .then(email_obj => {
      // console.log('Email Object:', email_obj)
...
Email.prototype.promise_leave_request_emails = function(args) {
  ...
  const promise_email_to_supervisor = ({
    ...
    // console.log('email to supervisor context:', context)
